...
# This target _could_ run both `pip install` commands unconditionally because
# `pip install` is idempotent if versions have not changed. The benefits of
# checking the version number before installing are that if there's nothing to
# do then (a) it's faster and (b) it produces less noisy output.
...
# Add new dependencies to requirements/development.txt whenever pyproject.toml
# changes
